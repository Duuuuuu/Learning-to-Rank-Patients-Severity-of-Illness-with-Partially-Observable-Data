...
def gradient_descent(theta, Ss, zs, C, alphazs, eta=0.01, iters=1000, lam=1, mu=1, print_every=10):
	gradident_fun = grad(objective)
	for i in range(iters):
		theta -= eta * gradident_fun(theta, Ss, zs, C, alphazs, lam, mu)
...
def dops(X, Y, T, C, m, alpha, init, eta=0.01, iters=1000, lam=1, mu=1, print_every=10):
	...
	# optimize
	theta = gradient_descent(init, Ss, zs, C, alphazs, eta, iters, lam, mu, print_every)
