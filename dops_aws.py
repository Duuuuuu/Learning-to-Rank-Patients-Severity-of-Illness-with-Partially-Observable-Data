...
def sto_gradient_descent(theta, Ss, zs, C, alphazs, maxzs, batch_size=10, eta=0.01, iters=1000, lam=1, mu=1, print_every=10, cover_indices=[]):
	...
	for i in range(iters):
		...
		for S, maxz in zip(Ss, maxzs):
			if max([f_theta(theta, s, C, cover_indices)[0] for s in S]) == maxz:
				score += 1
		if score > best_score:
			best_score = score
			best_theta = theta
...
def dops(X, Y, T, C, m, alpha, init, batch_size=10, eta=0.01, iters=1000, lam=1, mu=1, print_every=10,cover_indices=[]):
	# config
	M, d = X.shape
	N = int(np.floor(M/m))
